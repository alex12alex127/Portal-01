...
async function getNotificheUtente(userId, limit = 20, offset = 0) {
  const result = await db.query(
    'SELECT id, tipo, titolo, messaggio, letta, created_at FROM notifiche WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3',
    [userId, limit, offset]
  );
  return result.rows;
}

async function contaNotificheTotali(userId) {
  const result = await db.query(
    'SELECT COUNT(*)::int AS count FROM notifiche WHERE user_id = $1',
    [userId]
  );
