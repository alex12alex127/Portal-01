  ...
  try {
    ...
    if (overlap.rows.length > 0) {
      return res.status(400).json({ 
        error: 'Hai già una richiesta per questo periodo' 
      });
    }
    ...
    console.log(`[${new Date().toISOString()}] Ferie request created by user ${req.session.userId}`);
...
router.get('/api/list', requireAuth, apiLimiter, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT * FROM ferie WHERE user_id = $1 ORDER BY data_inizio DESC',
      [req.session.userId]
