...
/**
 * Archivia/disarchivia una conversazione. Solo il creatore può farlo.
 * Archivia per TUTTI i partecipanti.
 */
async function archiviaConversazione(convId, userId, archivia = true) {
  // Verifica che l'utente sia il creatore
  const check = await db.query(
    'SELECT created_by FROM conversazioni WHERE id = $1',
    [convId]
  );
  if (check.rows.length === 0) throw new Error('Conversazione non trovata');
  if (check.rows[0].created_by !== userId) throw new Error('Solo il creatore può archiviare la conversazione');
  // Archivia per tutti i partecipanti
  await db.query(
    'UPDATE conversazione_partecipanti SET archiviata = $1 WHERE conversazione_id = $2',
    [archivia, convId]
  );
}
